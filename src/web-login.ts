...
<script>const statusPath=${encodedStatus};const verifyPath=${encodedVerify};const form=document.querySelector('#verify');const poll=async()=>{try{const r=await fetch(statusPath,{cache:'no-store'});const x=await r.json();document.querySelector('#status').textContent=x.message;form.hidden=!x.needsCode;if(x.refresh){window.location.reload();return}if(x.done)return}catch{}setTimeout(poll,1000)};void poll();form.addEventListener('submit',async e=>{e.preventDefault();const code=document.querySelector('#code').value.trim();const r=await fetch(verifyPath+'?code='+encodeURIComponent(code),{method:'POST',cache:'no-store'});const x=await r.json();document.querySelector('#status').textContent=x.message;if(x.ok)form.hidden=true})</script></html>`;
}
...
export async function webLogin(
...
): Promise<{ credentialPath: string; accountId: string }> {
  let session = await startLoginSession({
  ...
  const server: Server = createServer((request, response) => {
    ...
    if (request.url === "/status") {
      ...
      const message =
        failure !== undefined
          ? "登录失败，请刷新后重试。"
          : latest.status === "confirmed" || latest.status === "already-bound"
            ? "连接成功，可以关闭此页面。"
            : latest.status === "scanned"
              ? "已扫码，请在手机上确认。"
      ...
      response.end(
        JSON.stringify({
          ...
          ok:
            latest.status === "confirmed" || latest.status === "already-bound",
          message,
          refresh: latest.status === "expired",
        }),
      );
      return;
    }
    response.setHeader("content-type", "text/html; charset=utf-8");
    if (latest.status === "expired") latest = { status: "waiting" };
  ...
  if (address === null || typeof address === "string")
    throw new Error("could not determine browser login address");
  ...
  try {
    while (Date.now() < deadline && options.signal?.aborted !== true) {
      ...
      if (latest.status === "expired") {
        session = await startLoginSession({ apiBase: options.apiBase, signal: options.signal });
        continue;
      }
      ...
      if (latest.status === "already-bound") {
        ...
        if (adopted === undefined)
          throw new Error(
            "Weixin confirmed an existing binding, but no usable local credential was found",
          );
